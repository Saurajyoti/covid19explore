...
!['covid 19 virus image'](C:/Users/sk3586/Documents/git_startup/covid-19_saura/covid19_2871-2560x852.png)\
...
path <- 'C:/Users/sk3586/Documents/git_startup/COVID-19/csse_covid_19_data/csse_covid_19_time_series'
setwd(path)
...
d6_usa <- d6_usa %>%
  mutate(confirmed_per_1M = confirmed/population*1e6)
...
ggplot() + 
  ...
  geom_map(aes(fill=confirmed_per_1M, map_id = FIPS),
           data = d6_usa, map=county_map, color="palevioletred3", size=0.05) + 
  
  scale_fill_gradient(low="green", high="red",
                      name = "Confirmed per million of county",
...
```{r plot confirmed cases per unit density of each county (for US mainland)}
d6_usa <- d6_usa %>%
  mutate(confirmed_per_density = confirmed/density)
d6_usa
...
ggplot() + 
  ...
  geom_polygon(data=us.map,
                aes(x=long, y=lat, group=group), fill="gray94", color="gray25") +
  geom_map(aes(fill=confirmed_per_density, map_id = FIPS),
           data = d6_usa, map=county_map, color="palevioletred3", size=0.05) + 
