...
p <- ggplot(d3) +
  ...
  theme(legend.position="none")
setwd(path)
ggsave("cmi_vs_n_log10_bycounty.png", p, dpi=900)

p <- ggplot(d3) +
  geom_smooth(aes(cmi, n_confirmed_log, color = state_name), se = F, size = .08, method = lm) +
  theme_classic() +
  theme(legend.position="none")
setwd(path)
ggsave("cmi_vs_n_log10_bystate.png", p, dpi=900)
